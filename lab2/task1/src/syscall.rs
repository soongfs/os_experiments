...
use crate::{TaskInfo, EFAULT, EINVAL, ENOSYS, SYS_GET_TASKINFO, SYS_SHUTDOWN, SYS_WRITE};
...
pub fn write(bytes: &[u8]) -> isize {
    ...
    invoke_syscall3(SYS_WRITE, bytes.as_ptr() as usize, bytes.len(), 0)
}

#[inline(never)]
pub fn get_taskinfo(task_info: *mut TaskInfo) -> isize {
    invoke_syscall3(SYS_GET_TASKINFO, task_info as usize, 0, 0)
}

#[inline(never)]
pub fn shutdown(code: u32) -> ! {
    let _ = invoke_syscall3(SYS_SHUTDOWN, code as usize, 0, 0);
    loop {
        unsafe {
