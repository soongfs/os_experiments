...
const USER_STAGE_BEFORE_SELFMAP_PROBE: u64 = 0xfeed_face_0000_0005;
const USER_ECALL_FAILURE_CODE: u64 = 0xbad0_5e1f_0000_0005;

const SELF_ALIAS_INDEX: usize = 510;
const SELF_REF_INDEX: usize = 511;
...
const KERNEL_SELFMAP_FLAGS: usize = PTE_R | PTE_W | PTE_A | PTE_D;
const SELFMAP_PROBE_VA: usize =
    sv39_canonical_va(SELF_REF_INDEX, SELF_REF_INDEX, SELF_ALIAS_INDEX, 0);
...
impl UserEvidence {
    fn collect() -> Self {
        ...
        Self {
            ...
            unexpected_selfmap_value: shared.unexpected_selfmap_value,
...
pub extern "C" fn start_supervisor() -> ! {
    ...
    build_page_tables_with_selfmap();
    activate_single_address_space();
    prepare_user_frame();

    let kernel_direct = walk_virtual(KERNEL_PROBE_VA);
    let user_text_direct = walk_virtual(USER_TEXT_VA);
    let user_data_direct = walk_virtual(USER_DATA_VA);
    let user_stack_direct = walk_virtual(USER_STACK_VA);
    let selfmap_probe_direct = walk_virtual(SELFMAP_PROBE_VA);

    let kernel_selfmap = selfmap_walk_virtual(KERNEL_PROBE_VA);
    let user_text_selfmap = selfmap_walk_virtual(USER_TEXT_VA);
    let user_data_selfmap = selfmap_walk_virtual(USER_DATA_VA);
    let user_stack_selfmap = selfmap_walk_virtual(USER_STACK_VA);
    let selfmap_probe_selfmap = selfmap_walk_virtual(SELFMAP_PROBE_VA);

    let root_self_direct = unsafe { ROOT_PAGE_TABLE.entries[SELF_REF_INDEX] };
    let root_self_via_selfmap = selfmap_root_pte(SELF_REF_INDEX);
    let root_alias_via_selfmap = selfmap_root_pte(SELF_ALIAS_INDEX);

    println!("[kernel] booted in S-mode with Sv39 enabled");
    println!("[kernel] LAB4 kernel task5 recursive self-mapping");
    println!(
        "[kernel] selfmap policy: root[{}] recursively points to root_pa; reserved alias index {} exposes table pages as kernel-only leaf mappings under fixed Sv39 virtual patterns",
        SELF_REF_INDEX, SELF_ALIAS_INDEX
    );
    println!(
    ...
    println!(
        "[kernel] selfmap addresses: root_table_va={:#x} low_l1_table_va={:#x} user_l0_table_va={:#x} kernel_l1_table_va={:#x} probe_va={:#x}",
        SELFMAP_ROOT_TABLE_VA,
        selfmap_l1_table_va(vpn2_index(USER_TEXT_VA)),
        selfmap_l0_table_va(vpn2_index(USER_TEXT_VA), vpn1_index(USER_TEXT_VA)),
        selfmap_l1_table_va(vpn2_index(KERNEL_PROBE_VA)),
        SELFMAP_PROBE_VA
    );
    println!(
        "[selfmap] root[ref={}] direct={:#018x} via_root_table_va={:#018x} expected={:#018x}",
        SELF_REF_INDEX,
        root_self_direct,
        root_self_via_selfmap,
        table_pte(root_page_table_pa())
    );
    println!(
        "[selfmap] root[alias={}] via_root_table_va={:#018x} flags={}",
        SELF_ALIAS_INDEX,
        root_alias_via_selfmap,
        PteFlags(root_alias_via_selfmap)
    );

    print_walk("kernel_probe_direct", KERNEL_PROBE_VA, kernel_direct);
    print_walk("user_text_direct", USER_TEXT_VA, user_text_direct);
    print_walk("user_data_direct", USER_DATA_VA, user_data_direct);
    print_walk("user_stack_direct", USER_STACK_VA, user_stack_direct);
    print_walk("selfmap_probe_direct", SELFMAP_PROBE_VA, selfmap_probe_direct);

    print_selfmap_walk("kernel_probe_via_selfmap", KERNEL_PROBE_VA, kernel_selfmap);
    print_selfmap_walk("user_text_via_selfmap", USER_TEXT_VA, user_text_selfmap);
    print_selfmap_walk("user_data_via_selfmap", USER_DATA_VA, user_data_selfmap);
    print_selfmap_walk("user_stack_via_selfmap", USER_STACK_VA, user_stack_selfmap);
    print_selfmap_walk(
        "selfmap_probe_via_selfmap",
        SELFMAP_PROBE_VA,
        selfmap_probe_selfmap,
    );

    println!(
        "[kernel] direct/selfmap equality: kernel_probe={} user_text={} user_data={} user_stack={} selfmap_probe={}",
        pass_fail(kernel_direct == kernel_selfmap),
        pass_fail(user_text_direct == user_text_selfmap),
        pass_fail(user_data_direct == user_data_selfmap),
        pass_fail(user_stack_direct == user_stack_selfmap),
        pass_fail(selfmap_probe_direct == selfmap_probe_selfmap)
    );
    println!(
...
pub extern "C" fn handle_supervisor_trap(frame: &mut TrapFrame) {
    ...
    match scause {
        LOAD_PAGE_FAULT => handle_expected_selfmap_fault(frame, stval),
...
fn handle_expected_selfmap_fault(frame: &TrapFrame, stval: usize) -> ! {
    ...
    let root_self_expected = table_pte(root_page_table_pa());
    let root_self_direct = unsafe { ROOT_PAGE_TABLE.entries[SELF_REF_INDEX] };
    let root_self_via_selfmap = selfmap_root_pte(SELF_REF_INDEX);
    let root_alias_via_selfmap = selfmap_root_pte(SELF_ALIAS_INDEX);

    println!(
        "[kernel] trapped user selfmap probe: scause={:#x} sepc={:#x} stval={:#x} satp={:#x}",
        LOAD_PAGE_FAULT,
        frame.epc,
        stval,
        read_satp()
    ...
    println!(
        "[kernel] user evidence: seed={:#018x} readback={:#018x} stack_echo={:#018x} stage={:#018x} unexpected_selfmap_value={:#018x} unexpected_syscall={:#018x}",
        evidence.seed,
        evidence.readback,
        evidence.stack_echo,
        evidence.stage_marker,
        evidence.unexpected_selfmap_value,
    ...
    let self_entry_ok = root_self_direct == root_self_expected
        && root_self_via_selfmap == root_self_expected
        && pte_to_pa(root_self_direct) == root_page_table_pa()
        && !pte_has_user(root_self_direct);
    let selfmap_walk_ok = kernel_direct == kernel_selfmap
        && user_text_direct == user_text_selfmap
        && user_data_direct == user_data_selfmap
        && user_stack_direct == user_stack_selfmap
        && selfmap_probe_direct == selfmap_probe_selfmap;
    let safety_ok = stval == SELFMAP_PROBE_VA
        && evidence.readback == evidence.seed
        && evidence.stack_echo == evidence.seed
        && evidence.stage_marker == USER_STAGE_BEFORE_SELFMAP_PROBE
        && evidence.unexpected_selfmap_value == 0
        && evidence.unexpected_syscall_code == 0
        && !pte_has_user(root_alias_via_selfmap)
        && !pte_has_user(selfmap_probe_direct.leaf_pte);

    println!(
        "[kernel] acceptance root[{}] points to root_pa and matches recursive readback: {}",
        SELF_REF_INDEX,
        pass_fail(self_entry_ok)
    );
    println!(
        "[kernel] acceptance fixed-pattern selfmap virtual addresses traverse root/L1/L0 entries: {}",
        pass_fail(selfmap_walk_ok)
    );
    println!(
        "[kernel] acceptance user selfmap probe is blocked by U=0 kernel-only aliases: {}",
        pass_fail(safety_ok)
    );

    qemu_exit(if self_entry_ok && selfmap_walk_ok && safety_ok {
        0
    } else {
        1
    })
...
fn handle_unexpected_user_ecall(frame: &TrapFrame, stval: usize) -> ! {
    ...
    println!(
        "[kernel] expected failure path would have used a0={:#x} a7={}",
        USER_ECALL_FAILURE_CODE,
        SYS_FAIL
    );
    println!(
        "[kernel] this means the user probe executed past the forbidden selfmap access"
    );
    qemu_exit(1)
}
...
fn initialize_user_pages() {
    ...
    zero_page(ptr::addr_of_mut!(USER_DATA_PAGE));
    ...
    shared.unexpected_selfmap_value = 0;
    shared.stack_echo = 0;
    shared.unexpected_syscall_code = 0;
}

fn build_page_tables_with_selfmap() {
    ...
    unsafe {
        ...
        ROOT_PAGE_TABLE.entries[SELF_REF_INDEX] = table_pte(root_page_table_pa());
        ROOT_PAGE_TABLE.entries[SELF_ALIAS_INDEX] =
            leaf_pte(root_page_table_pa(), KERNEL_SELFMAP_FLAGS);
        ...
        LOW_L1_PAGE_TABLE.entries[SELF_ALIAS_INDEX] =
            leaf_pte(low_l1_table_pa(), KERNEL_SELFMAP_FLAGS);

        DEV_L0_PAGE_TABLE.entries[vpn0_index(QEMU_TEST_BASE)] =
            leaf_pte(QEMU_TEST_BASE, PTE_R | PTE_W | PTE_A | PTE_D);
        DEV_L0_PAGE_TABLE.entries[SELF_ALIAS_INDEX] =
            leaf_pte(dev_l0_table_pa(), KERNEL_SELFMAP_FLAGS);
        ...
        USER_L0_PAGE_TABLE.entries[SELF_ALIAS_INDEX] =
            leaf_pte(user_l0_table_pa(), KERNEL_SELFMAP_FLAGS);
        ...
        KERNEL_L1_PAGE_TABLE.entries[SELF_ALIAS_INDEX] =
            leaf_pte(kernel_l1_table_pa(), KERNEL_SELFMAP_FLAGS);
    }
}
...
fn print_selfmap_walk(name: &str, va: usize, walk: WalkResult) {
    let root_entry_va = selfmap_root_pte_va(walk.vpn2);
    let mut l1_entry_va = 0;
    let mut l0_entry_va = 0;

    if pte_is_valid(walk.root_pte) && !pte_is_leaf(walk.root_pte) {
        l1_entry_va = selfmap_l1_pte_va(walk.vpn2, walk.vpn1);
        if pte_is_valid(walk.l1_pte) && !pte_is_leaf(walk.l1_pte) {
            l0_entry_va = selfmap_l0_pte_va(walk.vpn2, walk.vpn1, walk.vpn0);
        }
    }

    println!(
        "[selfmap] {} va={:#x} entry_vas(root={:#x}, l1={:#x}, l0={:#x}) level={} root_pte={:#018x} l1_pte={:#018x} l0_pte={:#018x} leaf_pte={:#018x} leaf_pa={:#x} flags={}",
        name,
        va,
        root_entry_va,
        l1_entry_va,
...
fn selfmap_walk_virtual(va: usize) -> WalkResult {
    let mut result = WalkResult::zeroed();

    result.vpn2 = vpn2_index(va);
    result.vpn1 = vpn1_index(va);
    result.vpn0 = vpn0_index(va);
    result.root_pte = selfmap_root_pte(result.vpn2);
    ...
    result.l1_pte = selfmap_l1_pte(result.vpn2, result.vpn1);
    ...
    result.l0_pte = selfmap_l0_pte(result.vpn2, result.vpn1, result.vpn0);
    result.leaf_level = 0;
    result.leaf_pte = result.l0_pte;
    result
}

fn selfmap_root_pte(index: usize) -> usize {
    unsafe { ptr::read_volatile(selfmap_root_pte_va(index) as *const usize) }
}

fn selfmap_l1_pte(vpn2: usize, index: usize) -> usize {
    unsafe { ptr::read_volatile(selfmap_l1_pte_va(vpn2, index) as *const usize) }
}

fn selfmap_l0_pte(vpn2: usize, vpn1: usize, index: usize) -> usize {
    unsafe { ptr::read_volatile(selfmap_l0_pte_va(vpn2, vpn1, index) as *const usize) }
}

fn selfmap_root_pte_va(index: usize) -> usize {
    SELFMAP_ROOT_TABLE_VA + index * core::mem::size_of::<usize>()
}

fn selfmap_l1_pte_va(vpn2: usize, index: usize) -> usize {
    selfmap_l1_table_va(vpn2) + index * core::mem::size_of::<usize>()
}

fn selfmap_l0_pte_va(vpn2: usize, vpn1: usize, index: usize) -> usize {
    selfmap_l0_table_va(vpn2, vpn1) + index * core::mem::size_of::<usize>()
}

const SELFMAP_ROOT_TABLE_VA: usize =
    sv39_canonical_va(SELF_REF_INDEX, SELF_REF_INDEX, SELF_ALIAS_INDEX, 0);

const fn selfmap_l1_table_va(vpn2: usize) -> usize {
    sv39_canonical_va(SELF_REF_INDEX, vpn2, SELF_ALIAS_INDEX, 0)
}

const fn selfmap_l0_table_va(vpn2: usize, vpn1: usize) -> usize {
    sv39_canonical_va(vpn2, vpn1, SELF_ALIAS_INDEX, 0)
}

const fn sv39_canonical_va(vpn2: usize, vpn1: usize, vpn0: usize, offset: usize) -> usize {
    let va39 = (vpn2 << 30) | (vpn1 << 21) | (vpn0 << 12) | offset;
    if (va39 & (1usize << 38)) != 0 {
        va39 | (!((1usize << 39) - 1))
    } else {
        va39
