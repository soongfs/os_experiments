...
const ENABLE_PROCESS_SWITCH_TRACE: bool = true;
const PROCESS_SWITCH_TRACE_LIMIT: usize = 12;

const YIELD_PROCESS_ROUNDS: usize = 3;
const YIELD_PROCESS_SPIN: u64 = 45_000;
const TIMESLICE_PROCESS_PHASES: usize = 3;
const TIMESLICE_PROCESS_SPIN: u64 = 1_250_000;
...
pub extern "C" fn start_kernel() -> ! {
    ...
    println!("[kernel] LAB5 kernel task1 process-switch tracing");
    println!(
        "[kernel] timer source: mtime={:#x}, mtimecmp={:#x}, frequency={} Hz, 1 tick={} ns",
        MTIME_ADDR, MTIMECMP_ADDR, MTIME_FREQ_HZ, MTIME_TICK_NS
    );
    println!(
        "[kernel] process switch trace: enabled={}, limit={} record(s)",
        ENABLE_PROCESS_SWITCH_TRACE,
        PROCESS_SWITCH_TRACE_LIMIT
    );
    println!(
        "[kernel] workloads: pid=1 performs {} explicit yields, pid=2 runs {} long compute phases for timer preemption",
        YIELD_PROCESS_ROUNDS,
        TIMESLICE_PROCESS_PHASES
    );
...
fn handle_finish(code: u64, frame: &mut TrapFrame) {
    ...
    println!(
        "[kernel] process exit: pid={} name={} code={:#x}",
...
fn emit_switch_trace(
    ...
    if !ENABLE_PROCESS_SWITCH_TRACE {
        return;
    }

    let should_emit = unsafe { SWITCH_TRACE_EMITTED < PROCESS_SWITCH_TRACE_LIMIT };
    if !should_emit {
        return;
    }

    println!(
        "[sched] switch#{:02} save_done: from pid={} name={} reason={} saved_mepc={:#x}",
        switch_index,
        from_id,
        from_name,
        reason.as_str(),
    ...
    println!(
        "[sched] switch#{:02} restore_begin: to pid={} name={} reason={} next_mepc={:#x}",
        switch_index,
        to_id,
        to_name,
        reason.as_str(),
    ...
    unsafe {
        ...
        if SWITCH_TRACE_EMITTED == PROCESS_SWITCH_TRACE_LIMIT {
            println!(
...
fn log_initial_restore(task_index: usize) {
    ...
    println!(
        "[sched] boot restore_begin: to pid={} name={} reason={} next_mepc={:#x}",
...
fn finish_experiment() -> ! {
    ...
    println!("[kernel] process switch summary begins");
    println!(
        "[kernel] summary: total_switches={} explicit_yield_switches={} time_slice_switches={} process_exit_switches={} timer_interrupts={}",
        total_switches,
        explicit_yields,
        time_slices,
        exit_switches,
    ...
    for index in 0..TASK_COUNT {
        ...
        println!(
            "[kernel] process result: pid={} name={} exit_code={:#x}",
            TASK_DEFS[index].id,
    ...
    println!(
        "[kernel] acceptance: switch logs came from scheduler/switch_to path with pid-tagged save_done + restore_begin markers"
    );

    let pass = explicit_yields > 0 && time_slices > 0 && total_switches >= 3;
    println!(
        "[kernel] acceptance yield reason observed: {}",
        pass_fail(explicit_yields > 0)
    );
    println!(
        "[kernel] acceptance time_slice reason observed: {}",
        pass_fail(time_slices > 0)
    );
    println!(
        "[kernel] acceptance pid-based readable switch trace captured: {}",
...
pub extern "C" fn yield_demo_entry() -> ! {
    ...
    while round < YIELD_PROCESS_ROUNDS {
        syscall::yield_now();
        acc = busy_mix(acc, YIELD_PROCESS_SPIN);
        round += 1;
    }
...
pub extern "C" fn timeslice_demo_entry() -> ! {
    ...
    while phase < TIMESLICE_PROCESS_PHASES {
        acc = busy_mix(acc, TIMESLICE_PROCESS_SPIN);
        phase += 1;
    }
