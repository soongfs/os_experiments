...
const FP_CONTEXT_STRATEGY: &str = "eager";
...
pub extern "C" fn start_kernel() -> ! {
    ...
    println!("[kernel] LAB3 kernel task3 floating-point context + preemption support");
    println!(
        "[kernel] timer source: mtime={:#x}, mtimecmp={:#x}, frequency={} Hz, 1 tick={} ns",
        MTIME_ADDR, MTIMECMP_ADDR, MTIME_FREQ_HZ, MTIME_TICK_NS
    );
    println!(
        "[kernel] fp trap frame: {} bytes, includes f0-f31 ({}) + fcsr",
        size_of::<TrapFrame>(),
        32 * size_of::<u64>()
    );
    println!(
        "[kernel] fp context strategy: {} save/restore on every trap entry/exit",
        FP_CONTEXT_STRATEGY
    );
    println!(
...
fn finish_experiment() -> ! {
    ...
    println!(
        "[kernel] acceptance concurrent floating-point checksums match reference exactly: {}",
        pass_fail(all_match)
    );
    println!(
        "[kernel] acceptance eager fp save/restore preserved f0-f31 and fcsr across preemption: {}",
        pass_fail(all_match)
    );
    println!(
