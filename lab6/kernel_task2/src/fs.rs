use crate::abi::{
    FsStat, StatDiagnostics, EEXIST, EFBIG, EINVAL, EISDIR, ENAMETOOLONG, ENOENT, ENOSPC,
    ENOTDIR, ENOTEMPTY, FS_BLOCK_SIZE, FS_DEVICE_ID, FS_DIRECT_POINTERS, FS_KIND_DIR,
    FS_KIND_FILE, FS_LEVEL_DIRECT, FS_LEVEL_DOUBLE, FS_LEVEL_SINGLE, FS_LEVEL_TRIPLE,
    FS_MAX_DIR_ENTRIES, FS_NAME_MAX, FS_PATH_MAX, FS_POINTERS_PER_BLOCK,
};
...
static mut STAT_DIAGNOSTICS: StatDiagnostics = StatDiagnostics::empty();
...
pub fn init() {
    unsafe {
        ...
        NEXT_PTR_BLOCK = 0;
        METADATA_CLOCK_US = 1;
        STAT_DIAGNOSTICS = StatDiagnostics::empty();
...
pub fn stat_diagnostics() -> StatDiagnostics {
    unsafe { STAT_DIAGNOSTICS }
}

pub fn note_stat_fault(error: isize) {
    unsafe {
        STAT_DIAGNOSTICS.stat_calls += 1;
        STAT_DIAGNOSTICS.failed_lookups += 1;
        STAT_DIAGNOSTICS.last_error = error as i64;
    }
}
...
pub fn stat(path: &[u8], stat: &mut FsStat) -> isize {
    ...
    let inode_index = match lookup_path(path) {
        ...
        Err(err) => {
            unsafe {
                STAT_DIAGNOSTICS.failed_lookups += 1;
                STAT_DIAGNOSTICS.last_error = err as i64;
            }
            return err;
        }
    };
    ...
    unsafe {
        STAT_DIAGNOSTICS.successful_lookups += 1;
        ...
        STAT_DIAGNOSTICS.successful_copyouts += 1;
        STAT_DIAGNOSTICS.last_inode_number = stat.inode_number;
        STAT_DIAGNOSTICS.last_kind = stat.kind as u64;
        STAT_DIAGNOSTICS.last_size_bytes = stat.size_bytes;
        STAT_DIAGNOSTICS.last_error = 0;
    }
