use crate::abi::{
    ...
    FS_BLOCK_SIZE, FS_DIRECT_POINTERS, FS_KIND_DIR, FS_KIND_FILE, FS_LEVEL_DIRECT,
...
pub fn list_dir(path: &[u8], dst: &mut [u8]) -> isize {
    ...
    unsafe {
        ...
        for child_slot in 0..child_count {
            ...
            dst[written..written + name_len].copy_from_slice(&INODES[child_index].name[..name_len]);
            written += name_len;
            dst[written] = b'\n';
            written += 1;
        }
